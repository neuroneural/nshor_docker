...
########## HIGH MOTION CHECK ##########
mean_fwd=`framewise_displacement ${mocodir}/fmri_ts_ds_mc_vr_motion.1D`
echo "mean_fwd: $mean_fwd"
thr=0.2
if (( $(echo "$mean_fwd > $thr" |bc -l) )); then
	echo "high motion detected"
        processed_filename="${subjectID}_${base}_hm".nii.gz
fi
########## HIGH MOTION CHECK ##########
