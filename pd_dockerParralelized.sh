...
function skullstrip() {
   ...
   # N4BiasFieldCorrection -d 3 -i ${subIDpath}${subjectID}_T1_01.nii.gz -o ${anatdir}/${subjectID}_T1w_bc.nii.gz 
    ...
    N4BiasFieldCorrection -d 3 -i ${subIDpath}/anat/${subjectID}_run-01_T1w.nii.gz -o ${subIDpath}/anat/${subjectID}_run-01_T1w_bc.nii.gz
    echo "anatdir" ${anatdir}
    echo "subjectID" ${subjectID}
    cd /ROBEX
    ./ROBEX ${subIDpath}/anat/${subjectID}_run-01_T1w_bc.nii.gz ${anatdir}/${subjectID}_T1w_bc_ss.nii.gz
}
...
#vepi=${subIDpath}/func/${subjectID}_task-rest_run-01_bold.nii.gz
